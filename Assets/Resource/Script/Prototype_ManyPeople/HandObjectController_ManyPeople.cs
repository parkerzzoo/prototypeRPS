...
public class HandObjectController_ManyPeople : Singletone<HandObjectController_ManyPeople>
{
    ...
    public void OnEndRound(List<UserData> userList, HandType frontHandType, float duration)
    {
        ...
        for (int i = 0; i < _sortedList.Count; i++)
            if (_sortedList[i] != null && !_sortedList[i].isAlive)
                _loserCount++;
    ...
    {
        float _delay = duration / (float)loserCount;
        for (int i = 0; i < handObjectList.Length; i++)
        {
            if(userList[i] != null && !userList[i].isAlive)
            {
