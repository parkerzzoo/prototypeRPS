...
public class FlowControl_FewPeople : FlowControl
{
	...
	protected override void OnConnected()
	{
		...
		GameController.Instance.UIControl<UIControl_FewPeople>().FadeOutCoverImage();
	}
	...
	protected override void OnUserListChange(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnUserListChange(_userList);
	}
	...
	protected override void OnStartGame(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnStartGame(_userList);
	}
	...
	protected override void OnStartRound(JSONObject data)
	{
		...
		for (int i = 0; i < _userList.Count; i++)
		{
			...
			{
				GameController.Instance.UIControl<UIControl_FewPeople>().ShowCenterTextPanel("손을 선택해주세요!", 0f, _duration);
	...
	IEnumerator OnStartRoundCor(List<UserData> userList, float duration)
    {
		...
		if(duration > _readyPlayTime)
        {
			...
			GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnStartRound(userList, _readyPlayTime);
		}
		else
        {
			yield return new WaitForSeconds(0.18f);
			GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnStartRound(userList, duration);
		}
	}
	...
	protected override void OnEndRound(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnEndRound(_userList, _frontHand, _duration);
	}
	...
	protected override void OnEndGame(JSONObject data)
	{
		...
		for (int i = 0; i < _userList.Count; i++)
		{
			if (_userList[i].IsMe)
			{
				GameController.Instance.UIControl<UIControl_FewPeople>().UpdateChipCountText(_userList[i].point);
				break;
			}
		}
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnEndGame(_userList);
	}
	...
	protected override void OnResetRound(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnResetRound(_userList);
	}
	...
	protected override void OnResetGame(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnResetGame(_userList);
	}
	...
	protected override void OnUserSpeak(JSONObject data)
	{
		...
		GameController.Instance.UIControl<UIControl_FewPeople>().CreatChatObject(_msg);
	}
	...
	protected override void OnUserHandChange(JSONObject data)
	{
		...
		GameController.Instance.HandObjectControl<HandObjectControl_FewPeople>().OnUserHandChange(_userList);
	}

	protected override void OnConnectError()
    {
		GameController.Instance.UIControl<UIControl_FewPeople>().ActiveConnectErrorPanel();
	}
